...
public class CarTransferController {
    ...
    @SecurityRequirement(name = "bearerAuth")
    @Operation(summary = "Update a car transfer")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successful operation",
                    content = {@Content(mediaType = "application/json",
                            schema = @Schema(implementation = CarTransferResponse.class))}),
            ...
            @ApiResponse(responseCode = "404", description = "Not Found",
                    content = {@Content(mediaType = "application/json",
                            schema = @Schema(implementation = RecordNotFoundErrorModel.class))})})
    public ResponseEntity<CarTransferResponse> update(@Parameter(description = "Car ID",
            content = @Content(schema = @Schema(type = "integer", format = ""))) @PathVariable Long id,
                                                      @Parameter(description = "Transfer ID",
                                                              content = @Content(schema = @Schema(
                                                                      type = "integer", format = "")))
                                                      ...
                                                      @RequestBody @Valid CarTransferRequest carTransferRequest) {
        CarTransfer carTransfer = carTransferConverter.toCarTransfer(carTransferRequest);
        CarTransfer updatedCarTransfer = carTransferService.update(id, tid, carTransfer);
        CarTransferResponse carTransferResponse = carTransferConverter.toCarTransferResponse(updatedCarTransfer);
        return ResponseEntity.status(HttpStatus.OK).body(carTransferResponse);
    }
